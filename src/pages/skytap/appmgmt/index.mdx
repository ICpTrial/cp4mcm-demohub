...
![image](https://user-images.githubusercontent.com/22209835/113955299-68ee7700-9856-11eb-8cd9-853d1931a7b0.png)  
   ...
   1. レポジトリ情報として、以下の情報を指定してください。
        |項目|値|備考|
        --|--
        |URL|https://github.com/ibmuser-cp4mcmlab/ModernResort| | 
        |UserName|（パブリック・レポジトリですので不要です）|
        |AccessToken|（パブリック・レポジトリですので不要です）|
        |Branch|ibmuserXX|XXは 01-06のセミナー管理者によってアサインされた番号です|
        |Path|（指定不要）|
...
1. Ingress に指定された ホスト名 modresort.10.0.0.2.nip.io でアクセスしてみます。
...
1. ブラウザで、 [**modresort**のGitページ](https://github.com/ibmuser-cp4mcmlab/ModernResort) を開きます。
3. アサインされた ibmuserXX のブランチを開きます（ここを間違えると、ハンズオンを実施している他の人の定義を変更してしまいますのでご留意ください)。
![image](https://user-images.githubusercontent.com/22209835/115247874-25eba800-a162-11eb-8d96-5db7925b89a7.png)
1. **modresort-deployment.yaml**の定義を開き、右上のメニューにある 鉛筆アイコンをクリックして、編集モードに入ります。ログインが求められたら ibmuserとして指定されたパスワードでログインします。
![image](https://user-images.githubusercontent.com/22209835/115248318-8b3f9900-a162-11eb-96c6-f43e0454df26.png)
...
本来はGitの権限分離や承認フローの仕組みを回して、Mainのブランチに変更を反映しますが、今回は演習ですので割愛です。
...
それでは、ここでは、PlacementRule に定義されたラベルを変更することによって、アプリケーションの配置先クラスターを変更します。
