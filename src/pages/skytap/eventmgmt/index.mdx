...
1. モニタリングコンポーネントのイベント管理機能を利用するユーザー・グループを作成します。  
...
１． 他のユーザーも追加してきましょう。デモ環境では LDAPがすでに統合されていますので、LDAPにすでに存在するメンバーを Cloud Pa
4. するために、まずユーザーをシステムに追加しましょう。
5. 
6. Before we create support group, let's start with adding a user to the system. The demo environment has an LDAP configured, so you will import user from the LDAP.
