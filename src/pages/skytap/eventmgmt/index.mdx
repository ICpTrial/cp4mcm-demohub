...
　　ここでは自動的に処理を呼び出します。**Automation**アイコンをクリックして、事前定義したrunbook自動化の定義を指定します。
    1. Provide the name and secription. Below, there is an editor allowing you to define a runbook content. You can define multiple steps, provide commands to be run along with the guidence for the operator who will use this later. You can also call automations and this is exactly what we want. Click the **Automation** icon to add previously defined automation to the runbook
   ![](images/2020-11-16-16-16-19.png)
    1.. '+' sign next to **Start_Online_Banking** automationの横にある`+`ボタンをクリックして、自動的に実行するrunbookを追加します。確認のポップアップでは、**Apply**をクリックします。
...
1. 最後に、イベントとrunbook 作業手順書を関連づけます。これは イベント・ポリシー によって実行されます。
    1. モニタリングの管理画面にもとり、**Policies**を選びます。 先ほどは、ここで Slack通知のためのIncident Policyを定義しました。今回は イベントへの対応を定義するEvent Policyを作成します。**New event policy**をクリックします。
    ![](images/2020-11-16-16-25-02.png)
    1. 名前と説明を記載します。
    ![](images/2020-11-16-16-29-17.png)
    1. イベントセクションでに、**Specify Condtions**を選び、**Condition 1**として、
    In the events section select **Specify conditions** and for **Condition 1**, change the attribute to **Summary**, select **"contains"** for the operator, and enter **"node process not running"** を入力します・ue.
    ![](images/2020-11-16-16-31-01.png)
    条件を作成したら、ミスがない検証することが可能です。
    1. **Action** セクションまでスクロール・ダウンします。**Assign runbook**を選び、**Add runbook assignment**をクリックします。
    ![](images/2020-11-16-16-32-45.png)
    1. 利用可能なrunbookを選択肢、**Apply**をクリックします。そして、**Automatically run this runbook** のチェックボックスにチェックを入れて、**Save**します。
    ![](images/2020-11-16-16-35-49.png)
    1. さいごに Poilicy を **Enabled** にします。
...
1. モニタリングの管理ページに戻り、**Integrations**をクリックします。
    1. サンプル・イベントを生成します。
    1. モニタリングの管理ページに戻り、**Incidents**を選択し、**All incidents**を表示します。
   ![](images/2020-11-16-17-07-45.png)
    **Online Banking Application**に関連するインシデントが自動的に `Support Group 01`にアサインされていることが分かります。**Investivate**をクリックします。

    1. **Resolution**問題解決ビューで、Slack通知が送付されていることを確認します。
    1. また自動的に EventPolicyによって、runbook（作業手順書）が実行されていることを確認します。
   ![](images/2020-11-16-17-10-57.png)
    1. デスクトップの **Management Hub**ターミナルを開き、 定義したジョブの実行結果である **rba-result.txt**を探します。
   ![](images/2020-11-16-17-12-30.png)
    1. もし**rba-result.txt**が見つからなければ、Ansible Towerの executionログを確認してください。

   ![](images/2020-11-16-17-15-35.png)

   ![](images/2020-11-16-17-15-44.png)

   異常で、イベント通知およびRuntime Automationのハンズオンは終了です。
