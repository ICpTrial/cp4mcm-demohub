...
IBM Cloud Pak for Multicloud Management に含まれる Event Manager および Runbook Automationの機能は、今後その他のAI関連コンポーネントと統合され、**IBM Cloud Pak for WatsionAIOps**に進化していきます。**IBM Cloud Pak for WatsonAIOps**は、以下のような機能を提供します。
* AIManager ログの自然言語解析 を通じた 異常検知 （いつもと違うメッセージを自動的に検知して通知）
* Metric Manager による 機械学習による異常検知 （いつもと違うメトリックの挙動を検知して通知）
* Event Manager による 多様なイベントの関連付けの絞り込みと、Runbook Automation
