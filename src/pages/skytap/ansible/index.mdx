...
IBM Cloud Pak for Multicloud Managementには、初期構成の自動化や 日々の管理運用を自動化するために利用することができる **Ansible Automation** が統合されてています。  
**Ansible Automation**の集中管理コンポーネントである **Ansible Tower**は、IBM Cloud Pak for Multilcloud Managementの管理ハブ上で稼働しており、連携して利用していくことが可能です。

この Chapter 2 では、Chapter 1 で統合した環境の 初期セットアップに Ansible を利用していきます。
