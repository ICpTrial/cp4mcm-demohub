...
1. ダッシュボードには、Cloud Pak に統合されている管理対象が表示されています。Cloud Provider に １つOpenShift Cluster が統合され、VMware環境のVMインスタンスが４つ統合されているのが確認できます。
このように Kubernetes環境と、従来型仮想化基盤の仮想マシン（Instarstructure VM)およびIaaSの仮想マシン(CloudVM)　双方を管理できるのが Cloud Pak for Multicloud Management のポイントです。 
![image](https://user-images.githubusercontent.com/22209835/113707957-9da9e380-971b-11eb-89e2-ff96111536a2.png)
1. すでに統合されているクラスターを確認していきます。メニューの Automate Infrastructure (基盤の自動化） から Clusters を開きます。
...
### 既存クラスターのインポート
  ...
  ログインできない場合は、Skytap環境のメニューの 鍵のマークをクリックして、admin のパスワードを Insertして下さい。
