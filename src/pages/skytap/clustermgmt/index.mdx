...
1. 黄色の**MicroK8s**のターミナルで、以下のコマンドで名前空間を確認します。
    ...
    ```
    **open-cluster-management-agent** および **open-cluster-management-agent-addon** の名前空間が生成されていることに留意ください。
    **open-cluster-management-agent** はクラスター登録のためのエージェント、**open-cluster-management-agent-addon**は クラスター管理のためのエージェントが自動的に導入され、**管理ハブクラスター** とコミュニケーションを開始しています。  
...
2. サーチ・メニュー は 管理対象のクラスターの アプリケーション・リソースとKubernetesオブジェクトに対するアクセスを提供します。非常に強力な機能で、 パブリック・クラウド、プライベート・クラウドどこで稼働していても、容易に I確認が必要なリソースを見つけることが可能です。サーチ・オーバービューはのダッシュボードには、よく利用される検索フィルターが事前定義されています。これらのフィルターは、特定のアプリケーションに特化したフィルター、特定のクラスターに特化したフィルターなど、利用者のニーズに応じて作成して保管しておくことが可能です。  
ここでは **Created Last Hour**のボタンをクリックします。
![image](https://user-images.githubusercontent.com/22209835/113952400-91737280-9850-11eb-8f50-31d8ec206dae.png)

3. **Pod**セクションをスクロールダウンしていくと、先ほど作成した**demo-modresort**Podが見えるはずです。 フィルター・セクションに search criteria を追加することで、さらに絞り込むことも可能です。
...
この章では、IBM Cloud Pak for Multicloud Management をもちいて、以下の内容を確認しました。
